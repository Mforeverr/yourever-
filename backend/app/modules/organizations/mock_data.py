"""Development-only organization payloads used as a backend fallback.

These records mirror the placeholder tenants that already live in the
frontend (`src/lib/mock-users.ts`). When the feature flag
``enable_mock_organization_fallback`` is enabled (typically only in local
development) the service can respond with these names so the Workspace Hub
doesn't break while the real persistence layer is missing. They are *not*
intended to surface demo data to production users; disabling the flag
removes any exposure.
"""
