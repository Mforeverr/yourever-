...
export function ModernSocialButton({
  provider,
  onClick,
  isLoading = false,
  disabled = false,
  text
}: ModernSocialButtonProps) {
  const defaultText = {
  ...
  return (
    <Button
      ...
      variant="outline"
      className={cn(
        'w-full relative overflow-hidden transition-all duration-200 hover:shadow-md',
        'border-2 hover:border-primary/50 active:scale-[0.98]',
...
export function AuthSwitch({ mode, onModeChange, className }: AuthSwitchProps) {
  return (
